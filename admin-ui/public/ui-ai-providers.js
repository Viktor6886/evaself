...
async function providerAction(action, id, routeArg = null, pinArg = null) {
  ...
  if (action === "delete") {
    askConfirm({
      ...
      // Прежний текст обещал запрет, которого больше нет: удаление
      // активной конфигурации сначала передаёт её роль модели, которая
      // уже стоит основной в разговоре. Не удаляется только последняя:
      // без неё Ева останется без модели.
      description: "Если конфигурация активна, её роль перейдёт к основной модели разговора. "
        + "Единственную конфигурацию удалить нельзя. Сохранённые агенты не изменятся.",
      expected: "DELETE",
      action: async () => {
