 ...
 * те же внутренние маршруты мимо ролей и аудита. Здесь тот же набор
 * операций работает под сессией панели: чтение доступно всем вошедшим,
 * правка настроек SDK — owner и admin, а личная переписка — после
 * подтверждения последствий и с записью в журнал событий.
 *
 ...
 * История диалога.
 *
 * Личная переписка: открывается только по явной кнопке и после
 * подтверждения последствий, каждое открытие попадает в журнал событий
 * (кто и чью, без текста). Пароль здесь не спрашивается — вход в панель
 * уже подтвердил, кто это.
 */
