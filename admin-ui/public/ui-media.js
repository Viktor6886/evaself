...
function mediaCheckReport(result) {
  const rows = [
    ...
    // По этому полю App Server решает, передавать ли изображение
    // провайдеру вообще. Пока каталог о зрении молчал, картинка
    // заменялась заглушкой, и до модели не доходила.
    ["Каталог объявляет зрение", result.catalog_vision === true
      ? "да"
      : result.catalog_vision === false
        ? "нет"
  ...
  const verdict = result.recognized
    ...
    : result.ok
      ? result.catalog_vision === false
        // Отдельный вердикт только тогда, когда каталог прямо сказал
        // «зрения нет»: это ровно то состояние, в котором роутер картинку
        // принимает, а до модели она не доходит.
        ? "Каталог роутера не объявляет зрение: App Server заменит изображение заглушкой, "
          + "и до модели оно не дойдёт. Проверьте, что в цепочке vision есть включённый "
          + "провайдер с поддержкой изображений."
