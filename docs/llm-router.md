...
Читать `cached_tokens_in` мало: пока в запрос не поставлены точки кэширования, провайдеру нечего отдавать из кэша, и доля остаётся нулевой при любом объёме постоянного префикса. У OpenAI-совместимых endpoint'ов кэш включается на их стороне сам; у Anthropic его задаёт поле `cache_control`, и без него постоянные тридцать тысяч токенов оплачиваются по полной ставке в каждом шаге каждого хода.

Включается по провайдеру, в `additional_parameters`:

```json
{ "prompt_cache": true }
{ "prompt_cache": true, "prompt_cache_ttl": "1h" }
```

Не переменной окружения, а полем провайдера — по двум причинам. Совместимость с Anthropic заявляют многие endpoint'ы, а разбирают тело строго не все: глобальный флаг ломал бы разом то, что работало. И канареечное включение на одном провайдере при глобальном флаге невозможно.

Ключи транспортные: в тело запроса провайдеру они не уходят.

Точек кэширования ставится две из четырёх разрешённых:

- на системном блоке. Порядок рендера у Anthropic — `tools`, `system`, `messages`, поэтому эта точка закрывает собой и описания инструментов: отдельная точка на них тратила бы место впустую;
- в конце истории — на последнем пригодном блоке последнего сообщения. Следующий шаг того же хода читает весь разговор из кэша и платит только за приращение. Блок размышления точкой не помечается: Anthropic его так не принимает.

**Срок.** Умолчание — пять минут: запись обновляется при каждом чтении, поэтому шагам одного хода, идущим подряд, этого хватает, и стоит она дешевле. `1h` вдвое дороже на запись и оправдан там, где между ходами человека проходят десятки минут. Что выбрать — показывает `make check-tokens`: если доля кэша высокая внутри хода и нулевая между ходами, разговоры идут реже пяти минут.

**Отказ.** Endpoint, не понявший `cache_control`, отвечает `400`. Такой отказ до человека не доходит: кэш выключается для этого провайдера до перезапуска, и запрос повторяется без него ровно один раз.

**Проверка.** `cached_tokens_in` больше нуля на повторных обращениях означает, что кэш работает. Ноль при включённой настройке — где-то в префиксе появилось меняющееся значение, и он перестал совпадать побайтно.
