...
/**
 * Одна человеческая строка поверх машинного отказа.
 *
 * Владелец, разговаривающий с Евой в своём чате, получал тысячу двести
 * знаков вложенного JSON — и должен был вычитывать из них, что
 * произошло. Самая частая причина при этом читается однозначно: роутер
 * говорит, во сколько токенов не поместился запрос.
 *
 * Возвращает `null`, когда сказать нечего: выдумывать объяснение по
 * незнакомой ошибке хуже, чем показать её как есть.
 */
export function explainFailure(message: string): string | null {
  const overflow = /не помещается в (\d+)/.exec(message);
  const requested = /примерно на (\d+) токен/.exec(message);
  if (overflow) {
    const limit = Number(overflow[1]);
    const size = requested ? Number(requested[1]) : null;
    return size !== null
      ? `История диалога переросла окно модели: ${size} токенов при пределе ${limit}.`
