...
/**
 * Обслуживание канонического контекста отложено, ничего не изменено.
 *
 * Не отказ и не ошибка исполнения: агент цел, право есть, но состояние
 * не позволяет писать безопасно — managed-файл MemFS изменён снаружи,
 * checkout не готов, или ход не отпустил агента в отведённое окно.
 * Отдельный код нужен, чтобы «не применили и ничего не тронули» не
 * смешивалось с «применили половину»: первое повторяется само, второе
 * требует разбирательства.
 */
export const maintenanceDeferred = (message: string, details?: unknown) =>
  new EvaError(message, {
