...
async function main(): Promise<void> {
  ...
  const skillIndexer = artifactRegistry ? new FilesystemSkillIndexer(artifactRegistry, { root: indexerRoots[0]!, runtimeRoot: "/data/letta/.skills", environment: "production" }) : null;
  const syncOnce = async (root: string) => {
    if (!artifactRegistry) return;
    const idx = new FilesystemSkillIndexer(artifactRegistry, { root, runtimeRoot: "/data/letta/.skills", environment: "production" });
    const r = await idx.sync();
    logger.info("Project skills synchronized", { root, ...r });
  };
  syncSkills = async () => { for (const root of indexerRoots) await syncOnce(root); };
  if (skillIndexer) await syncSkills();
