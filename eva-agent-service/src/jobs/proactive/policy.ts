...
export type ProactiveKind =
  ...
  | "daily_insight"
  | "weekly_review"
  // Сообщение в окно, которое выбрал сам человек: «пиши мне с
  // одиннадцати до двенадцати». Минута внутри окна выбирается заранее и
  // один раз (`windows.ts`).
  | "initiative";
...
const RULES: Record<ProactiveKind, { minSilenceMs: number; minIntervalMs: number; maxUnanswered: number }> = {
  ...
  initiative: {
    // Окно человек назначил сам, поэтому молчание его не отменяет: он
    // просил писать в это время, а не «когда я пропаду».
    minSilenceMs: 0,
    // Порог низкий намеренно. Окна человек расставил осознанно, и
    // отменять его выбор общим правилом «не чаще раза в три часа»
    // значит молча выбросить второе окно, которое он завёл нарочно.
    // Сорок пять минут защищают ровно от одного случая: два соседних
    // окна могут сработать на своей границе с разницей в минуту, и
    // тогда это уже не два разговора, а два сообщения подряд.
    minIntervalMs: 45 * 60_000,
    // Три неотвеченных подряд — это ответ. Порог выше, чем у heartbeat:
    // окна человек назначил осознанно, и молчать в ответ на два
    // сообщения — не то же самое, что не отвечать на непрошеное.
    maxUnanswered: 3,
  },
};
...
/**
 * Слот сообщения в окно.
 *
 * Окон у человека несколько, и все они живут в одних местных сутках,
 * поэтому дата слот не различает — различает окно. Иначе второе окно дня
 * упёрлось бы в слот первого и не сработало никогда.
 */
export function initiativeSlotKey(localDate: string, windowId: string | number): string {
  return `${localDate}:initiative:${windowId}`;
}
...
export function decideProactive(
  ...
  // сам назначил время. То же и с окном: человек выбрал этот час
  // осознанно, и подменять его выбор общим правилом «после десяти не
  // писать» значило бы отменить саму настройку. Остальные виды — наша
  // инициатива, и ночью её быть не должно.
  const chosenByUser = kind === "reminder" || kind === "initiative";
  if (!chosenByUser && inQuietHours(context.timezone, now, context.quietHours)) {
    return { send: false, reason: "quiet_hours" };
  }

  // Человек написал последним и ждёт ответа: инициатива поверх
  // незакрытой переписки выглядит так, будто его не услышали. Окна это
  // касается наравне с остальной инициативой — назначенное время не
  // повод перебить незакрытый разговор.
