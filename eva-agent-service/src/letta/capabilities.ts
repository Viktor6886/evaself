...
/**
 * Версия, на которой реестр подтверждён.
 *
 * Значение остаётся литералом, а не читается из `package.json` в рантайме:
 * тест сравнивает его с фактически установленным пакетом, и вычисляемая
 * версия соглашалась бы сама с собой. Но и набирать его руками больше не
 * нужно — литерал выводит из lock-файла `scripts/ci/sync-letta-versions.py`
 * вместе с версией образа App Server, и CI проверяет, что все три места
 * совпадают. Так обновление SDK доезжает целиком или не доезжает вовсе.
 *
 * Что установленный пакет действительно умеет обещанное, доказывает не эта
 * строка, а `letta-contract.test.ts`: он ищет каждый путь реестра на живых
