...
export class ResearchJobWorker {
  ...
  async run(context: JobContext): Promise<void> {
    ...
    try { await this.db.withUserScope({userId,label:"research.run",inherit:true}, async () => {
      ...
      const orchestrator=new ResearchOrchestrator({plan:async(query,maxQueries,signal)=>await structuredRetry({complete:async({repair})=>await this.options.router.complete({model:"eva/json",messages:[{role:"user",content:JSON.stringify({query,maxQueries})}],system_prompt:'Return strict JSON object {"queries":["..."]} with 3 to maxQueries distinct queries.',response_format:{type:"json_object"},metadata:{route:"json",sensitive:true},repair},signal),parse:(raw)=>{const value:unknown=JSON.parse(raw);const queries=(value as {queries?:unknown})?.queries;if(!Array.isArray(queries)||queries.length<3||queries.length>maxQueries||queries.some(x=>typeof x!=="string"||!x.trim()))throw new Error("research_query_plan_invalid");return queries as string[];}},[],{signal}),search:(q,s)=>web.search(q,s),read:(u,s,m)=>web.read(u,s,m),extract:async(content,signal)=>await structuredRetry({complete:async({repair})=>await this.options.router.complete({model:"eva/json",messages:[{role:"user",content}],response_format:{type:"json_object"},metadata:{route:"json",sensitive:true},repair},signal),parse:(raw)=>{const value:unknown=JSON.parse(raw);if(!isFacts(value))throw new Error("research_schema_invalid");return value;}},[],{signal}),save:async(report)=>{reportResult=report;}}, {maxQueries:Number(process.env.EVA_RESEARCH_MAX_QUERIES??3),maxSources:Number(process.env.EVA_RESEARCH_MAX_SOURCES??12),maxPagesPerDomain:Number(process.env.EVA_RESEARCH_MAX_PAGES_DOMAIN??2),timeoutMs:Number(process.env.EVA_RESEARCH_TIMEOUT_MS??120_000),tokenBudget:Number(process.env.EVA_RESEARCH_TOKEN_BUDGET??20_000),maxPageBytes:Number(process.env.EVA_RESEARCH_MAX_PAGE_BYTES??512_000),maxConcurrency:Number(process.env.EVA_RESEARCH_CONCURRENCY??4)});
