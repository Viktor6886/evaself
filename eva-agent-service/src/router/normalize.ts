...
/**
 * Повтор после пустого ответа — с бо́льшим допустимым output budget.
 *
 * `max_tokens` ограничивает всё, что модель порождает, включая
 * рассуждение. Рассуждающая модель тратит на него сотни токенов прежде,
 * чем напишет первое слово, и при тесном бюджете возвращает пустой текст.
 * Часть провайдеров сообщает при этом штатное завершение, а не `length`,
 * поэтому отличить тесноту от поломки по ответу нельзя — можно только
 * попробовать ещё раз с запасом.
 *
 * Без этого повтора ход упирался в потолок и уходил в отказ: провайдеру
 * повторяли тот же запрос с тем же бюджетом и получали ту же пустоту,
 * пока не кончались попытки и вся цепочка. Человек видел «не получилось
 * обработать сообщение», а причина была в одном числе.
 *
 * Возврат `null` означает, что запас исчерпан: у провайдера больше нет
 * разрешённого бюджета, и повторять смысла нет.
 */
export function raiseOutputBudget(
  request: LlmRequest,
  provider: Pick<ProviderProfile, "max_output_tokens">,
): LlmRequest | null {
  const ceiling = Math.max(0, Math.floor(provider.max_output_tokens));
  const current = Math.max(0, Math.floor(request.max_tokens));
  if (current >= ceiling) return null;
  // Шаг крупный: разница между 512 и 1024 не решает ничего, а лишний
  // заход стоит человеку ещё одного ожидания ответа.
  const next = Math.min(ceiling, Math.max(current * 4, MIN_RAISED_BUDGET));
  return next > current ? { ...request, max_tokens: next } : null;
}

/** Ниже этого рассуждающая модель не успевает дойти до ответа. */
const MIN_RAISED_BUDGET = 2048;
...
export const BACKUP_PERSONA_DIRECTIVE = [
  ...
  // Резерв не видит memory block с персоной, поэтому род называется
  // здесь прямо: без этого при переключении провайдера Ева посреди
  // разговора начинала говорить о себе в мужском роде.
  "Ева — женское имя, и о себе ты говоришь только в женском роде:",
  "«поняла», «приняла», «сделала», «готова», «рада». О человеке — в его роде.",
