...
export class SdkSettingsManager {
  ...
  /**
   * Вывести настройки заново и объявить их Letta.
   *
   * Бюджет контекста выводится из включённых провайдеров, а вывод шёл
   * только при старте сервиса. На чистой установке провайдеров в этот
   * момент ещё нет — `default_context_window` оставался пустым, и Letta
   * до перезапуска не знала предела вовсе: агенты создавались без него,
   * а первая активация объявляла полное физическое окно модели.
   *
   * Зовёт это тот, кто состав провайдеров и меняет, — активация
   * (`LlmManager`). Второго места, которое выводит бюджет, при этом не
   * появляется: вывод как был здесь, так и остался.
   */
  async refresh(): Promise<void> {
    await this.applyRuntime(await this.db.getSdkSettings());
  }
   ...
   * задавал.
   *
   * Предел контекста складывается из двух разных вещей. Безопасность —
   ...
   * ничего, Letta не знала, когда сжимать контекст, и диалог рос до
