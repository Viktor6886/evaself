  ...
  build(tool: ToolBuilder): AnyAgentTool[] {
    return [
      tool(
        ...
        "Только читает актуальный тариф, срок, оставшиеся дни и остатки всех лимитов за сутки, неделю и месяц для текущего собеседника. Вызывай при каждом вопросе о подписке, оплате или квотах: эти данные нельзя брать из памяти и нельзя изменять.",
        objectSchema({}),
