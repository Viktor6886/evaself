...
export class CoreToolFactory {
    ...
    const tools: AnyAgentTool[] = [
      ...
      tool(
        ...
        "Ищет актуальную информацию через локальный приватный SearXNG. Возвращает ссылки"
          ...
          + " Для погоды указывай category=weather, для новостей — category=news и"
          + " time_range, для местного результата — language языка разговора.",
        objectSchema(
          {
  ...
  private async search(
    ...
    for (const step of ladder) {
      ...
      // Пустота из-за отказавших движков сужениями не лечится: снимать
      // фильтр у того, кто не ответил, бессмысленно, а каждая ступень
      // стоит целого круга ожидания. Три круга по двадцать секунд — это
      // минута на один вызов инструмента, и ход упирался в свой потолок
      // раньше, чем успевал что-нибудь сделать.
      if (found.unresponsive.length > 0) break;
    }
