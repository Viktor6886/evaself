...
test("наречие между «я» и сказуемым не прячет мужскую форму", () => {
  assert.equal(fix("Я немного запутался."), "Я немного запуталась.");
  assert.equal(fix("Прости, я не так понял."), "Прости, я не так поняла.");
  assert.equal(fix("Я всегда готов выслушать."), "Я всегда готова выслушать.");
  assert.equal(fix("Я рядом и готов слушать."), "Я рядом и готова слушать.");
  assert.equal(fix("Я был бы рад помочь."), "Я была бы рада помочь.");
  assert.equal(fix("Да, был неправ."), "Да, была неправа.");
  assert.equal(fix("Был рад помочь."), "Была рада помочь.");
  // За запятой «не» открывает косвенный вопрос о другом подлежащем.
  assert.equal(fix("Я подумал, не пришёл ли поезд."), "Я подумала, не пришёл ли поезд.");
});

test("сказуемое за коротким словом с запятой — тоже Ева о себе", () => {
  assert.equal(fix("Хорошо, записал."), "Хорошо, записала.");
  assert.equal(
    fix("Отлично, сохранил цель и поставил напоминание."),
    "Отлично, сохранила цель и поставила напоминание.",
  );
  assert.equal(fix("Кстати, нашёл интересную статью."), "Кстати, нашла интересную статью.");
  // Вопрос к человеку, чужое подлежащее и придаточное — не её.
  for (const text of [
    "Хорошо, понял?", "Хорошо, сказал он.", "Да, пришёл ответ от врача.",
    "Ну, ты понял.", "Спасибо, что написал.",
  ]) {
    assert.equal(fix(text), text);
  }
});

test("ряд за сказуемым, уже стоящим в женском роде, доводится до конца", () => {
  assert.equal(fix("Поняла тебя и записал."), "Поняла тебя и записала.");
  assert.equal(fix("Я поняла и записал цель."), "Я поняла и записала цель.");
  assert.equal(fix("Готова помочь и рад поддержать."), "Готова помочь и рада поддержать.");
  assert.equal(fix("Была рада помочь и написал план."), "Была рада помочь и написала план.");
  // Подлежащее за сказуемым — чужое, какой бы ни была форма.
  for (const text of [
    "Была хорошая погода, и пришёл он.", "Одна из причин — усталость.",
    "Была тишина, и раздался звук.", "Была весна, и запел скворец.",
    "Была осень, и пожелтел лист.", "Была вечеринка, и пришёл гость.",
    "Была причина, и объяснил её врач.", "Была проблема, и начался сбой.",
    "Да, был сбой.", "Конечно, был дождь.", "Был сбой на сервере.",
    "Начала работы не было, и сделал он.",
  ]) {
    assert.equal(fix(text), text);
  }
});
