...
const spec = {
  ...
  budget: { maxTokens: 1000, maxDurationMs: 60_000, maxCostMicros: 10_000 },
  instruction: "Подведи итоги недели",
...
test("agent job возвращает структурированный результат и отвергает мусор", () => {
  const good = parseProposal(
    '```json\n{"kind":"reflection","empty":false,"confidence":0.7,'
      + '"items":[{"kind":"reflection","ref":"node-1","summary":"Стало легче с планированием"}]}\n```',
    spec as never,
  );
  assert.equal(good.ok, true);
  assert.equal(good.ok === true && good.proposal.items[0]?.ref, "node-1");
  assert.equal(good.ok === true && good.proposal.items[0]?.confidence, 0.7);

  // Пустое предложение — валидный успех, а не отказ.
  const empty = parseProposal('{"kind":"reflection","empty":true,"confidence":0.9,"items":[]}', spec as never);
  assert.equal(empty.ok, true);
  assert.equal(empty.ok === true && empty.proposal.empty, true);

  for (const [reply, code] of [
    ["Вот что я думаю: всё хорошо", "agent_result_not_json"],
    ['{"kind":"research","empty":false,"confidence":1,"items":[]}', "agent_result_kind_unknown"],
    ['{"kind":"reflection","confidence":5,"items":[]}', "agent_result_confidence_invalid"],
    ['{"kind":"reflection","confidence":1,"items":[{"summary":""}]}', "agent_result_item_empty"],
  ] as const) {
    const parsed = parseProposal(reply, spec as never);
    assert.equal(parsed.ok, false, reply);
    assert.equal(parsed.ok === false && parsed.code, code);
  }
});

test("agent job не принимает больше предложений, чем объявлено", () => {
  const items = Array.from({ length: 4 }, (_, index) => ({
    kind: "reflection",
    summary: `итог ${index}`,
  }));
  const parsed = parseProposal(
    JSON.stringify({ kind: "reflection", empty: false, confidence: 0.5, items }),
    spec as never,
  );
  assert.equal(parsed.ok, false);
  assert.equal(parsed.ok === false && parsed.code, "agent_result_too_many_items");
});
