...
test("занятый лимит откладывает доставку и не тратит попытку", async () => {
  ...
  const limits = {
    take: async () => ({ waitMs: 2000, reason: "chat" as const }),
    cooldown: async () => {},
  };
...
test("429 с retry_after откладывает доставку на названный срок и не тратит попытку", async () => {
  ...
  const paused: number[] = [];
  const limits = {
    take: async () => ({ waitMs: 0, reason: null }),
    cooldown: async (_chat: number, seconds: number) => {
      paused.push(seconds);
    },
  };
  ...
  internal.transport = {
    deliver: async () => {
      const error = new Error("Too Many Requests") as Error & { response?: unknown };
      error.response = { ok: false, error_code: 429, parameters: { retry_after: 25 } };
      throw error;
    },
  };
  ...
  assert.deepEqual(paused, [25], "пауза не передана общему лимиту");
});
