...
test("служебный зачин запрещён, а род собеседника отделён от рода Евы", async (t) => {
  ...
  for (const [name, text] of [["персона", persona], ["системный промпт", systemPrompt]] as const) {
    ...
    // Умолчание в любую сторону — это выбор рода за человека. Женщина,
    // о которой пишут «ты сам», получает ровно ту ошибку, ради которой
    // правило и заводилось, — только в другую сторону.
    assert.match(text, /не угадывай/i, `${name}: угадывание рода не запрещено`);
    assert.match(
      text,
      /такая же ошибка|ошибается ровно так же/i,
      `${name}: мужское умолчание не названо ошибкой наравне с женским`,
    );
    assert.ok(
      !/по умолчанию в мужском/i.test(text),
      `${name}: мужской род вернулся умолчанием`,
    );
  }
});
...
test("местоположение со слов человека важнее профиля, а маршрут не выдумывается", async (t) => {
  ...
  // Разговор верит человеку, а расписание не может: `save_task` отвергает
  // пояс, отличный от подтверждённого, и считает стенное время по профилю.
  // Без этой оговорки правило «верь человеку» ставит напоминание не в тот
  // час — молча и ровно тогда, когда на него рассчитывают.
  assert.match(persona, /`save_task`/, "персона не называет инструмент задач");
  assert.match(persona, /`upsert_user_profile_field`/, "персона не называет путь смены пояса");
  for (const [name, text] of [["персона", persona], ["системный промпт", systemPrompt]] as const) {
    assert.match(
      text,
      /напоминани\p{L}*|отложенн\p{L}*|задач\p{L}*/iu,
      `${name}: расписание не отделено от разговора`,
    );
    assert.match(text, /молча/i, `${name}: молчаливая постановка задачи не запрещена`);
  }
});
...
test("расклад дня попадает в current_state, а не остаётся в переписке", async (t) => {
  ...
  // Недатированный блок переживает ночь и назавтра читается как
  // сегодняшний: вчерашние пары становятся сегодняшними. Ролловера у
  // `current_state` нет, поэтому дату блок обязан носить на себе.
  assert.match(skill, /Дата в\s+заголовке обязательна/i, "блок дня не датируется");
  assert.match(skill, /`local_date`/, "навык не сверяет дату блока с датой хода");
  assert.match(persona, /`local_date`/, "персона не сверяет дату блока с датой хода");
