...
-- pro → max.
--
-- Порядок важен: сначала подписки, потом квоты, иначе между двумя UPDATE
-- нашёлся бы момент, когда у подписки на pro нет квот.
--
-- Удаление перед переименованием — не перестраховка. Миграции
-- накатываются заново на каждом обновлении, а миграция 002 засевает
-- квоты pro с ON CONFLICT DO NOTHING: на втором проходе строк pro уже
-- нет (эта миграция их переименовала), конфликтовать нечему, и они
-- появляются снова. Следующий UPDATE тогда упирается в уже
-- существующую пару max — уникальный индекс по (plan, metric, period)
-- отвергает переименование, и обновление установки падает.
--
-- Удаляются только те строки pro, для которых max уже есть: настройки
-- владельца при этом остаются в max, а повторно засеянный pro исчезает
-- как дубль, которым он и является.
DELETE FROM quotas q
 WHERE q.plan = 'pro'
   AND EXISTS (
       SELECT 1 FROM quotas m
        WHERE m.plan = 'max' AND m.metric = q.metric AND m.period = q.period
   );
