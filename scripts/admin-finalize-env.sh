...
awk -v marker="$marker" '
  function managed(name) {
    return name ~ /^(EVA_(TELEGRAM|LLM|EMBEDDING|AGENT|SCHEDULER|HEARTBEAT|PROFILE|VECTOR|GRAPH|CONVERSATION|OUTBOX)_|LETTA_|CRAWL4AI_|SEARXNG_SECRET|LAVA_WEBHOOK_|POSTGRES_SUPER_PASSWORD|EVA_DB_PASSWORD|EVA_DB_READONLY_PASSWORD|LETTA_DB_PASSWORD|VALKEY_PASSWORD)/
  }
  {
    split($0, parts, "=")
    name = parts[1]
