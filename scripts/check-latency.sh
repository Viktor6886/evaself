...
if [ "${TURNS:-0}" -eq 0 ]; then
	...
	metric persona_sync_ms        "  сверка персоны"
...
cat <<'HINT'
  ...
  сверка персоны        — приведение агента к канонической персоне и
                          системному промпту. У совпавшей версии это
                          ноль. Ненулевая на КАЖДОМ ходе означает, что
                          сверка не доходит до конца: версия пишется
                          только после успеха, и попытка повторяется
                          снова и снова. Причина — в журнале, строка
                          «Canonical runtime context sync degraded».
