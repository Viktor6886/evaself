...
# ничего не поднимает.
#
# Бюджет — сколько контекста мы согласны оплачивать в каждом шаге, считая
# постоянный префикс. Он только опускает безопасный предел и живёт в коде
# (`CONTEXT_BUDGET_TOKENS` в `src/letta/context-window.ts`). Числа здесь
...
cat <<'HINT'
  ...
  safety_ceiling  — сколько физически примет самая слабая включённая
                    ...
                    поверх неё бюджет (`contextLimit`) опускает потолок до
                    того, что мы согласны платить в каждом шаге. Оба числа
                    считают ПОЛНОЕ окно, вместе с постоянным префиксом.
