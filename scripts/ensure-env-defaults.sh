...
# выбор, а наследство.
#
# Пока `EVA_LETTA_ADMIN_CLIENT` стоял выключенным, агенты, созданные
# раньше, оставались с персоной того дня: файл персоны правился, а блок
# в Letta — нет. Отсюда и мужской род в репликах. Переключается только
# ровно прежнее умолчание `false`; осознанно выставленное значение
# (например, `0` или комментарий рядом) не трогается.
# ---------------------------------------------------------------------
corrected=()
if grep -qxF 'EVA_LETTA_ADMIN_CLIENT=false' "$ENV_FILE"; then
	set_env EVA_LETTA_ADMIN_CLIENT true
	corrected+=("EVA_LETTA_ADMIN_CLIENT")
fi
