...
tar tzf "$ARCHIVE" >/dev/null 2>&1 || die "the archive is not a readable tar.gz"

STAGE="$(mktemp -d)"
trap 'rm -rf "$STAGE"' EXIT
tar xzf "$ARCHIVE" -C "$STAGE"
SRC="$(find "$STAGE" -maxdepth 1 -type d -name 'evaself-backup-*' | head -1)"
[ -n "$SRC" ] || die "the archive does not look like an Evaself backup"
...
if [ -f "$SRC/CHECKSUMS" ]; then
	if ( cd "$SRC" && sha256sum --quiet -c CHECKSUMS 2>/dev/null ); then
		...
		warn "checksum mismatch — the archive may be damaged"
		confirm "Continue anyway?" n || exit 1
	fi
fi

echo
warn "This replaces the databases, volumes and configuration of THIS installation."
...
if [ -f "$ENV_FILE" ] && docker volume inspect evaself_postgres_data >/dev/null 2>&1; then
	...
	if "$SCRIPT_DIR/backup.sh" >/dev/null 2>&1; then
		ok "current state saved (see ${BACKUP_DIR:-/var/backups/evaself})"
	else
...
[ -f "$SRC/config/versions.env" ] && cp "$SRC/config/versions.env" "$VERSIONS_FILE" && ok "versions.env restored"
[ -f "$SRC/config/Caddyfile" ] && cp "$SRC/config/Caddyfile" "$ROOT_DIR/Caddyfile" && ok "Caddyfile restored"
...
restore_volume() {
	...
	[ -f "$SRC/volumes/$file" ] || { info "no $file in the archive, skipping"; return 0; }
...
wait_for_health postgres 180 || die "postgres did not come up"

CID="$(compose ps -q backup-service)"
[ -n "$CID" ] || die "backup-service is not running"
